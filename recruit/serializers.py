...
from geminai import generate_job_description, generate_evaluation_criteria, call_gemini
...
class JobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Job
        fields = [
            'company_name', 'role', 'skills', 'project_experience',
            'other_details', 'job_description', 'evaluation_criteria', 'location'
        ]

    async def async_generate_details(self, company_name, role, skills, project_experience, other_details):
        try:
            ...
            evaluation_criteria = await generate_evaluation_criteria(
                {'role': role, 'company_name': company_name, 'skills': skills},
                job_description
            )
    ...
    def update(self, instance, validated_data):
        ...
        if not job_description or not evaluation_criteria:
            ...
            job_description, evaluation_criteria = loop.run_until_complete(
                self.async_generate_details(company_name, role, skills, project_experience, other_details)
            )
            loop.close()
